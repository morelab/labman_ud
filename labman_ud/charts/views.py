...
def funding_incomes_by_project(request, project_slug):
    ...
    project_incomes = FundingAmount.objects.filter(funding_id__in=funding_ids).values('year').annotate(
        total=Sum('own_amount'))
...
def projects_number_of_projects(request):
    ...
    for funding in fundings:
        for year in range(funding.project.start_year, funding.project.end_year + 1):
            projects_data[year][funding.project_id].add(
                geographical_scopes_by_id[funding.funding_program.geographical_scope_id])
    ...
    for scope, value_dict in default_project_dict.items():
        item_dict = {
            'key': str(inflection.titleize(scope)),
            'values': []
        }
        for year, count in value_dict.items():
...
def publication_coauthorships(request, max_position=None, within_group=False):
    ...
    # TODO this should be analyzed in order to make it work again with the new version of networkx
        ...
        if person_ids:
            for pos, person_id in enumerate(person_ids):
                for i in range(pos + 1, len(person_ids)):
                    ...
                    # TODO this should be reviewed in order to port networkx to python 3
                    if add_node:
...
def publications_egonetwork(request, author_slug):
    ...
    for relations in authors_per_publication.values():
        for (author_id1, name1), (author_id2, name2) in combinations(relations, 2):
            ...
            # TODO the current networx graph library allows to add weights directly in the add_edge method.
            G.add_edge(author_id1, author_id2)
    ...
    try:
        # TODO the analze class should be updated in order to meet the new standar of networkx
        G = nx_graph.analyze(G)
...
def cmp_members_by_start_date(member1, member2):
    for field in 'start_year', 'start_month', 'end_year', 'end_month':
        if member1[field] != member2[field]:
            return ((member1[field] > member2[field]) - (member1[field] < member2[field]))
...
def cmp_members_by_current_date(member1, member2):
    for field in 'end_year', 'end_month':
        if member1[field] != member2[field]:
            return ((member2[field] > member1[field]) - (member2[field] < member1[field]))
...
def cmp_members_by_length_current_first(member1, member2):
    ...
    return ((member2['length'] > member1['length']) - (member2['length'] < member1['length']))
...
def sort_members_by_together_time(members):
    def compare_members_length(m1, m2):
        """
        CPM to sort members by their length

        :param m1: member number 1
        :param m2: member number 2
        :return: cmp(x, y)
        """
        return (m2['length'] > m1['length']) - (m2['length'] < m1['length'])

    def compare_time_together(d1, d2):
        """
        CMP to compare time together

        :param d1: member 1 time together
        :param d2: member 2 time together
        :return: cmp(d1, d2)
        """
        return (d2[1] > d1[1]) - (d2[1] < d1[1])
    ...
    sorted_member_info = sorted(members, key=cmp_to_key(compare_members_length))
    ...
    for member1 in sorted_member_info:
        ...
        time_together_per_member[member1['member']].sort(key=cmp_to_key(compare_time_together))
    ...
    return list(map(lambda x: members_by_member[x], sorted_list))
...
def group_timeline(request):
    def compare(x, y):
        """
        A simple compare function to compare the x and y end.dates

        :param x: first date or defaulting to very new date
        :param y: second date or defaulting to very new date
        :return: cmp(x,y)
        """
        x_end_date = x.end_date or VERY_NEW_DATE
        y_end_date = y.end_date or VERY_NEW_DATE
        return (x_end_date > y_end_date) - (x_end_date < y_end_date)
    ...
    # TODO you need to compare why ana is in the first place
    for member in jobs_by_member:
        jobs_by_member[member].sort(key=cmp_to_key(compare))

    members = []
    members_by_id = {}
    for member, jobs in jobs_by_member.items():
    ...
    # In Python 3 the keys() method does not return a key list.
    sort_algorithm = request.GET.get('sort', list(algorithms.keys())[0])
    if sort_algorithm not in algorithms:
        sort_algorithm = list(algorithms.keys())[0]

    if sort_algorithm in ['Colleagues']:
        members = algorithms[sort_algorithm](members)
    else:
        members.sort(key=cmp_to_key(algorithms[sort_algorithm]))
...
def members_position_pie(request):
    ...
    member_jobs = Job.objects.filter(organization__in=organizations, person__is_active=True).select_related('person',
                                                                                                            'organization')
    ...
    for member in jobs_by_member:
        jobs_by_member[member].sort(lambda j1, j2: ((j1.end_date or VERY_NEW_DATE > j2.end_date or VERY_NEW_DATE)
                                                    - (j1.end_date or VERY_NEW_DATE < j2.end_date or VERY_NEW_DATE)))
...
def related_persons(request, person_slug, top):
    ...
    for pub_author in PublicationAuthor.objects.all().select_related('author__full_name').values('author__full_name',
                                                                                                 'publication'):
    ...
    sorted_relations = sorted(relations.items(), key=operator.itemgetter(1), reverse=True)[:50]
    # sorted_relations = filter(lambda (name, coef): coef > 0.075, sorted_relations)
    sorted_relations = filter(lambda coef: coef[1] > 0.075, sorted_relations)
